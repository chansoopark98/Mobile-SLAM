...
bool EigenQuaternionParameterization::Minus(const double* y, const double* x, double* y_minus_x) const {
    // Compute x_inverse (conjugate for unit quaternion in Eigen storage: x,y,z,w)
    double x_inv[4] = {-x[0], -x[1], -x[2], x[3]};
    double diff[4];
    EigenQuaternionProduct(x_inv, y, diff);

    // Convert quaternion difference to angle-axis
    // diff = (dx, dy, dz, dw) in Eigen convention
...
bool EigenQuaternionParameterization::MinusJacobian(const double* x, double* jacobian) const {
    // At x, the MinusJacobian is the pseudo-inverse of PlusJacobian
    // For unit quaternion: MinusJacobian = PlusJacobian^T (since PlusJacobian^T * PlusJacobian = I)
    // PlusJacobian is 4x3 (AmbientSize x TangentSize), row-major
    // MinusJacobian is 3x4 (TangentSize x AmbientSize), row-major
    jacobian[0] = x[3];
    jacobian[1] = -x[2];
    jacobian[2] = x[1];
    jacobian[3] = -x[0];
